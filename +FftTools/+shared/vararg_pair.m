...
import FftTools.shared.*

% KC EDIT - add condition to convert to struct first
...
%       KC EDIT - changed fail to error
% 		fail('unknown option name "%s"', arg)
        error('unknown option name "%s"', arg)
	end
end
...
try % typo in name should yield error
	opt = vararg_pair(opt, {'verify_bad!', 'new'}, 'allow_new', 0)
catch
	return
end
